...
app.controller('PostEditorController', ["$rootScope", "$scope", "$location", "$filter", "$log", "dataService", function ($rootScope, $scope, $location, $filter, $log, dataService) {
    ...
    $scope.save = function () {
        ...
        var summary = $('#form').find('#txtSummary').val();
        if (summary.length == 0) {           
            $('#txtSummary').parent().append('<label id="txtVerses-error1" class="error" for="txtSummary">  Summary is required </label>');
            $('#txtSummary').addClass('error');       
            return false;
        }
        spinOn();
